...
type ScheduleDeveloperId = Int32
...
data Schedule = Schedule {
    ...
    scheduleName :: String,
    scheduleDeveloperId :: Int32,
    scheduleClassId :: Int32
} deriving (Show)

instance Entity Schedule where
    create sched conn = schedule_create (scheduleDay sched) (scheduleNumber sched) (scheduleName sched) (scheduleDeveloperId sched) (scheduleClassId sched) conn
    update sched conn = schedule_update (scheduleId sched) (scheduleDay sched) (scheduleNumber sched) (scheduleName sched) (scheduleDeveloperId sched) (scheduleClassId sched) conn
...
schedule_create :: IConnection a => ScheduleDay -> ScheduleNumber -> ScheduleName -> ScheduleDeveloperId -> ScheduleClassId -> a -> IO Bool
...
schedule_create' day num name tid cid conn = do
    ...
    return $ commited == 1
    where
        query = "INSERT INTO schedule (day, number, name, developer_id, class_id) VALUES (?, ?, ?, ?, ?)"

schedule_update :: IConnection a => ScheduleID -> ScheduleDay -> ScheduleNumber -> ScheduleName -> ScheduleDeveloperId -> ScheduleClassId -> a -> IO Bool
...
schedule_update' sid day num name tid cid conn = do
    ...
    return $ commited == 1
    where
        query = "UPDATE schedule SET day = ?, number = ?, name = ?, developer_id = ?, class_id = ? WHERE id = ?"
...
schedule_all conn = do
    ...
    where
        ...
        unpuck [SqlInt32 sid, SqlByteString day, SqlInt32 num, SqlByteString name, SqlInt32 tid, SqlInt32 cid] = Schedule{scheduleId=sid,scheduleDay=(BS.unpack day),scheduleNumber=num,scheduleName=(BS.unpack name),scheduleDeveloperId=tid,scheduleClassId=cid}
...
schedule_find_id id conn = do
    ...
    return $ map unpuck rslt
    where
        query = printf "SELECT * FROM schedule WHERE id = %d" id
        unpuck [SqlInt32 sid, SqlByteString day, SqlInt32 num, SqlByteString name, SqlInt32 tid, SqlInt32 cid] = Schedule{scheduleId=sid,scheduleDay=(BS.unpack day),scheduleNumber=num,scheduleName=(BS.unpack name),scheduleDeveloperId=tid,scheduleClassId=cid}
        unpuck x = error $ "Unexpected result: " ++ show x
...
schedule_display' sched = do
    putStrLn ((printf "%d. " (scheduleId sched)) ++ (scheduleDay sched) ++ (printf " %d lesson, " (scheduleNumber sched)) ++ (scheduleName sched) ++ (printf " class %d, developer %d" (scheduleClassId sched) (scheduleDeveloperId sched)))